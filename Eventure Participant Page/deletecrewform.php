...
$deleteQuery = "DELETE FROM event_crews WHERE id = ? AND event_id = ? ";
...
if ($deleteStmt->execute() && $deleteStmt->affected_rows > 0) {
    ...
    if ($updateSlotsStmt->execute()) {
        echo "<script>alert('Crew application deleted successfully, and available slots updated.'); window.location.href='participantdashboard.php';</script>";
    } else {
        echo "<script>alert('Crew application deleted, but failed to update available slots.'); window.location.href='participantdashboard.php';</script>";
    }
} else {
    echo "<script>alert('Failed to delete crew application. Please try again.'); window.location.href='participantdashboard.php';</script>";
}
